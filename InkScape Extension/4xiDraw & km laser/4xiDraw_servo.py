...
class servo_gcode(inkex.Effect):
    ...
    def export_gcode(self,gcode):
        ...
        f.write(self.options.servo_up_command + " S" + str(int(self.options.servo_up)) + "\n" + self.header + "G1 F" + self.options.travel_speed + "\n" + gcode + self.footer)
        f.close()
    ...
    def __init__(self):
        ...
        self.OptionParser.add_option("",   "--servo-up-command",                action="store", type="string",          dest="servo_up_command",                    default="M05",                          help="Servo up gcode command")
        self.OptionParser.add_option("",   "--servo-down-command",              action="store", type="string",          dest="servo_down_command",                  default="M03",                          help="Servo down gcode end command")
        self.OptionParser.add_option("",   "--drawing-speed",                   action="store", type="int",             dest="drawing_speed",                       default="2500",                         help="Drawing speed (mm/min)")
        self.OptionParser.add_option("",   "--travel-speed",                    action="store", type="string",          dest="travel_speed",                        default="10000",                        help="Travel speed (mm/min)")
        self.OptionParser.add_option("",   "--servo-up",                        action="store", type="int",             dest="servo_up",                            default="0",                            help="S# is 256 for full up")
        self.OptionParser.add_option("",   "--servo-down",                      action="store", type="int",             dest="servo_down",                          default="90",                          help="S# is 256 or 10000 for full power")
        self.OptionParser.add_option("",   "--passes",                          action="store", type="int",             dest="passes",                              default="1",                            help="Quantity of passes")
        self.OptionParser.add_option("",   "--pass-depth",                      action="store", type="string",          dest="pass_depth",                          default="1",                            help="Depth")
        self.OptionParser.add_option("",   "--delay",                           action="store", type="string",          dest="delay",                               default="0.2",                          help="delay (ms)")
...
################################################################################
    def effect(self) :
        ...
        self.tools = {
            ...
            "id": "Drawing Machine",
            "penetration feed": self.options.drawing_speed,
            "feed": self.options.drawing_speed,
            "gcode before path": ("G4 P" + self.options.delay + "\n" + self.options.servo_down_command + " S" + str(int(self.options.servo_down)) + "\nG4 P" + self.options.delay),
            "gcode after path": ("G4 P" + self.options.delay + "\n" + self.options.servo_up_command + " S" + str(int(self.options.servo_up)) + "\n" + "G4 P" + self.options.delay + "\n" + "G1 F" + self.options.travel_speed),
        }
